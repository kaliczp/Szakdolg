...
Gyermekkoromban – történelem tantárgyból a honismeret
...
részletpontok attribútumait tartalmazó táblázathoz. Az épülettartozék
objektum azonosítószámát (~eptart_id~) és az objektum féleség kódját
(~obj_fels~) követően egy a táblázat további mezőinek jelentését előíró mező
...
megfelelően a ~geo_ae_id~ mezőbe az ~obj_kiterj~ kódjának megfelelő
...
#+CAPTION: A ~T_OBJ_ATTRCB~ épületek tartozékai objektumcsoport közös táblázata \parencite[30]{mihály18:m1}
...
programban is kezelhető. Ez azonban már nehezen kivitelezhető nagyobb
...
adatok közötti összefüggések leképezése nehézzé tette a
megvalósítást.
...
szoftver. Működés közben mutatja a programot a [[fig:freetrdat]]. ábra.
...
adatmegosztási elvárásoknak. Az említett implementációs nehézségek, az
...
befoglaló megadása (~gml:boundedBy~), amely pont esetében a pontot megadó
koordináta pár. Az első melléklet 18. sorától tanulmányozható, hogy a
...
geometriát jól beolvassa, de nem minden értéket, attribútumot tesz
...
formátum kezelésére, de a honlapjukon közzétettek egy nyilatkozatot,
...
A GML adatbázis, vagy akár XML fájl rugalmas építéshez némi
...
A fenti program nem túl bonyolult, de egy gml fájlhoz valós geometriát, s
hallgatónként eltérő környezetet és adatokat kellett volna
létrehozni. Együtt úgy éreztem meghaladja egy szakdolgozat kereteit a
feladat.  Szerencsére az ~libxml2~ több környezetben is
implementálásra került. Ezek közé tartozik az R programozási nyelv,
amelyet jól ismerek, s a fent említett kérdésekre már kész
megoldásokat használtam.
...
generálandó gml/XML fájlok – szempontjából szinte minden igényt lefed
az R.
...
Az XML csomagot elsősorban nem XML dokumentumok generálásra
fejlesztették. Ez is a funkciói közé tartozik, de fejlesztésének
célja, hogy az XML-ben hordozott adatok, információk hatékony
kezelését meg lehessen valósítani az R programban, S
nyelvben. Segítségével hatékonyan információt nyerhetünk a betöltött,
vagy az itt létrehozott XML dokumentumból. A fenti példában elmentett
XML dokumentumon demonstrálhatjuk a csomag, illetve az XML kezelésére
kitalált XPath lekérdezések hatékonyságát néhány soros kóddal
([[code:RXPath]]. kódrészlet).

#+ATTR_LATEX: :options numbers=left, numberstyle=\tiny\color{gray}
#+CAPTION: XPath lekérdezés R-ben
#+NAME: code:RXPath
#+begin_src R
  doc <- xmlParse("teszt.xml")
  TerTag <- getNodeSet(doc, "//terület")
  TerVals <- xmlValue(TerTag)
  sum(as.numeric(TerVals))
#+end_src

Az 1. sorban az ~xmlParse()~ függvénnyel visszaolvassuk az adatokat,
leképezzük az XML dokumentumot. A 2–4. sorokban kiszámoljuk az
adatbázisban tárolt területek összegét.  Először a ~getNodeSet()~
függvénnyel a teljes fán végighaladva a ~//terület~ XPath kifejezés
segítségével kigyűjtjük az összes terület tag-el rendelkező elemet és
elmentjük a ~TerTag~ objektumba. Majd a ~xmlValue()~ függvénnyel az
értékeket kigyűjtjük a ~TerVals~ objektumba. Ezket, mint minden
XML-ben tárolt adatot, alapértelmezetten karakteres kifejezésként
kapunk. Az ~as.numeric()~ függvény ezt számmá alakítja, amelyet
összegzünk a ~sum()~ függvénnyel (4. sor). A kifejezés eredménye az
összegzett területek.
...
Ebben a fejezetben program ismertetésénél ugyan azt a metódust követem, ahogy a
...
Első lépésként egy egyszerű, egyetlen földrészletet generáló
...
induló lépéssel kezdődik, amelyekben a program betölti az XML csomagot (4. sor),
A betöltés itt nem a korábban használt ~library()~ függvénnyel kezdődik,
...
oktatási céllal E-ING GML fájl generálása volt, ezért már az XML
formátum bemutatása is egy programozási példán keresztül történt. A
sok lehetőség közül két szoftvereszközt használtam erre. A ~libxml2~ C
...
készítettem. Ez a program csak egyetlen földrész előállítására képes. A
